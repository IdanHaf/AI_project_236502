...
class MSELogLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        # Calculate the base 10 logarithm of the MSE
...
for lr in lrs:
    for loss in losses:
        ...
        # Create lists to store validation losses for each epoch
        validation_losses = []
        ...
        # Save the model weights, training and validation losses for the current combination
        combination_dir = os.path.join(save_dir, f'lr_{lr}_loss_{type(loss).__name__}')
        if not os.path.exists(combination_dir):
            os.makedirs(combination_dir)

        model_save_path = os.path.join(combination_dir, 'model.pth')
        torch.save(model.state_dict(), model_save_path)

        val_loss_log_path = os.path.join(combination_dir, 'val_losses.npy')
...
best_model = torchvision.models.resnet50(weights='IMAGENET1K_V1')
...
best_model_path = os.path.join(save_dir, f'lr_{best_lr}_loss_{type(best_loss_func).__name__}', 'model.pth')
